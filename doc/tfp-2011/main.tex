...
operate on key/value pairs. In order to compare and better distinguish
...
(like channels) for transmitting computations. However, it does not
provide ready-made language features for fault tolerance, which is
...
offers primitives for code mobility, and the code for distribution of
computation and fault-tolerance has to be included by the user. It is
worth pointing out that an existing type-safe marshalling library
could still be used with Functory. \\ \textbf{can there exist an OCaml
  type-safe marshalling library without changes to the
  compiler?}. \\ \textbf{[Glasgow Distributed Haskell: TODO]}\\
Erlang~\cite{erlang} is a programming language which has features for
distribution and fault tolerance. In particular, it has features for
task distribution and is more well-known for its rich error detection
primitives and the ability to support hot-swapping. The error
detection primitives of Erlang allow nodes to monitor processes in
other nodes and also facilitate automatic migration of tasks in failed
nodes to recovered or active nodes. CamlP3l~\cite{camlP3l} mixes the
features of functional programming with predefined patterns for
parallel computation to offer a parallel programming
environment. Again, it is a programming language offering primitives
for distributing computation to parallel processes and also to merge
the results from parallel executions. The above mentioned languages
are programming languages themselves, our library, Functory, could be
implemented using any of the above. The main goal of Functory is to
provide the users of an existing functional programming language
high-level user-friendly APIs that hides the messy details of task
distribution and fault-tolerance, enabling a ready-to-use library for
...
optimize network traffic. However, Plasma MapReduce does not support
fault tolerance which is indespensible in any distributed computing
application. The Disco project~\cite{disco} is an implementation of
Google's MapReduce in Erlang~\cite{erlang}. Our library is not an
impementation of Google's MapReduce. A MapReduce implementation in
\Ocaml\ is Yohann Padioleau's MapReduce implementation in
\Ocaml~\cite{poor-man-mapreduce}.  It is built on top of
OCamlMPI~\cite{ocamlMPI}, while our approach uses a homemade protocol
for message passing.  Currently, we have less flexibility
