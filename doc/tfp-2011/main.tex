...
\begin{enumerate}
\item Distributed Functional Languages (DFLs) - functional
  languages that provide built-in primitives for
  distribution. Examples include ML5, JoCaml, Glasgow Distributed
  Haskell, Erlang, etc.
\item Libraries for existing and well-known functional programming
  languages that could be used in order to relieve the user from
  implementing intricate details of distribution like
  fault-tolerance, socket programming, etc.
\end{enumerate}
...
ML5~\cite{ML5}, a variant of ML, is a programming language for
distributed computing, specialized for web programming. It provides
primitives for transferring control between the client and the server,
as well as low-level primitives for marshalling the data. In
particular, it achieves type-safe marshalling. As in the case before,
ML5 is a programming language that offers primitives for code
mobility, rather than a user-friendly API for distribution of
computation and fault-tolerance.

Glasgow Distributed Haskell


The above mentioned languages are programming languages themselves,
our library, Functory, could be implemented using any of the above.
...
-----------  BELOW THIS IS THE OLD RELATED WORK --------
