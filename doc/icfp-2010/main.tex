...
language. 
Hence we designed and implemented such a
...
\item \emph{simplicity}: 
  switching between multiple cores and a network of
  machine is as simple as changing a couple of lines of code;
\item \emph{task distribution and fault-tolerance}: 
  it relieves the user from implementing task distribution routines
  and reliable fault-tolerance mechanism.
\end{itemize}
It is worth noting that the library is not
...
A typical distributed computing library
provides the following (borrowing some terminology from Google's
...
executable. In that case, we cannot marshal closures anymore but we
...
executed in parallel with other \ocaml{master}s. Function
\ocaml{master} always processes information locally, typically storing
it to internal data structures.
...
elements are done in parallel.  To facilitate such a processing, we
now derive the most commonly used list operations from our generic
API.
