...
Then the last three column show execution timings in seconds for the
...
\begin{center}
  \begin{tabular}{|r|r|r|r|r|}
    ...
    \\\hline
    19 &   19    &6120.0     &                       & 940.00 (6.51$\times$)  
    \\\hline
       &  306    &6130.0     &                       & 819.00 (7.48$\times$)
    \\\hline
  \end{tabular}
\end{center}
From the table above, it is clear that the \of{Cores} implementation
provides a significant speedup. When the number of tasks is large
enough, the speedup ratio is almost 8, which is also the number of
cores we use. 
% (When the number of tasks is small and not a multiple of
% the number of cores,  for example as in third row with 17 tasks
% executing on 8 cores, the

% TODO comment on network ratios

Where computation cost for each tasks is too small, then communication
cost dominates and could even slow down the execution
w.r.t. sequential implementation, as evident from second row.
...
We set a timeout limit of 600 seconds.
Each task completes with one of the four possible outcomes: \emph{valid},
\emph{invalid}, \emph{cannot-decide} (depending on
whether the VC is valid, invalid or undecided by the prover)
and \emph{timeout}.
...
% TODO compare total amount of time wrt sequential time
