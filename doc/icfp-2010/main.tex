...
\begin{itemize}
\item A notion of \emph{task} which denotes an atomic computation
...
contrary, can be impure and is never executed in parallel.
The \ocaml{master} function always processes
...
\vspace{-1em}
\begin{ocaml}
val worker_compute : ('a -> 'b) -> unit -> unit
val master_compute : 
  ('a * 'c -> 'b -> ('a * 'c) list) -> 
  ('a * 'c) list -> unit
\end{ocaml}
...
\vspace{-1em}
\begin{ocaml}
val worker_compute : (string -> string) -> unit -> unit
val master_compute : 
...
Let us assume that $f$ is defined as a toplevel function
\begin{ocaml}
  let f i = ...
\end{ocaml}
...
\of{compute} function, passing \of{f} as the worker function:
\begin{ocaml}
...
\begin{ocaml}
  open Poly
  let f i = ...
  let () = Worker.compute f ()
\end{ocaml}
The \of{Worker.compute} function enters a loop which waits for
tasks sent by the master and returns results computed using \of{f}.
...
\begin{ocaml}
  open Mono
  let f = ...
  let f_string i = string_of_int (f (int_of_string i))
  let () = Worker.compute f_string ()
\end{ocaml}
The master program is almost unchanged. We simply need to replace
\of{Poly} with \of{Mono} and to encode tasks as strings:
\begin{ocaml}
  let tasks = 
    let l = ref [] in 
    ...
    !l
\end{ocaml}
Consequently, \of{master} is now receiving a string as argument:
\begin{ocaml}
