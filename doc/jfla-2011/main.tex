...
$$\of{map}~x_i \oplus \dots \oplus \of{map}~x_j$$ a déjà été calculée
...
manière. Soit enfin il n'y a ni entrée pour $i-1$ ni entrée pour
...
\begin{ocaml}
  let map_fold_a ~map ~fold acc l =
    ...
    let worker = function Map x -> map x | Fold (x, y) -> fold x y in
...
présentés dans la table ci-dessous.
\begin{center}
  \begin{tabular}{|r|r|r|r|r|}
    \hline
...
milliards d'octets.
Une réalisation moins naïve pourrait voir les ouvriers lire les deux
matrices une seule fois, par exemple depuis un fichier, et le patron
envoyer uniquement des indices de lignes et de colonnes. Cela
...
déverminage). Calculer et stocker une telle information est immédiat,
