...
\begin{ocaml}
  let array_map f a =
    ...
    let b = if n = 0 then [| |] 
      else Array.create n (f a.(0)) in
    let tasks = 
      ref [] in for i = 1 to n - 1 do 
         tasks := (a.(i), i) :: !tasks 
      done;
    compute ~worker:f 
            ~master:(fun (_,i) bi -> 
...
val Master.compute : ('a * 'c -> 'b -> ('a * 'c) list) -> 
                     ('a * 'c) list -> unit
\end{ocaml}
...
val Master.compute : (string * 'c -> string -> 
                     (string * 'c) list) ->
                     (string * 'c) list -> unit
\end{ocaml}
\end{enumerate}
...
\begin{ocaml}
  val map_{local,remote}_fold : f:('a -> 'b) -> 
                               fold:('c -> 'b -> 'c) -> 
                               'c -> 'a list -> 'c   
\end{ocaml}
...
particuliers :
\begin{ocaml}
  val map_fold_{a,ac} : f:('a -> 'b) -> fold:('b -> 'b -> 'b) -> 
                        'b -> 'a list -> 'b 
\end{ocaml}
...
\begin{ocaml}
  let map_fold_a ~f ~fold acc l =
    let tasks = let i = ref 0 in 
         List.map (fun x -> incr i; 
                   Map x, (!i, !i)) l in
    let results = Hashtbl.create 17 in 
    let merge i j r = 
            ... traiter le résultat r du segment [i,j] ... in
    let worker = function Map x -> 
                 f x | Fold (x, y) -> fold x y in
    let master x r = match x with Map _,(i,_) -> 
             merge i i r | Fold _,(i,j) -> merge i j r in
    compute ~worker ~master tasks;
    try let _,_,r = Hashtbl.find results 1 in r with 
                    Not_found -> acc
\end{ocaml}
