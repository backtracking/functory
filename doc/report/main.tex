...
itself a task to be performed in parallel with \of{f} operations.
Thus tasks are of two different kinds and we use a sum type to make
the distinction.
When an intermediate \of{f} result is available, the
accumulator may be already in use in a \of{fold} operation. Hence we
need to store the intermediate \of{f} result in a table local to the
master, to be
used as soon as the accumulator becomes available.
The \of{master} function distinguishes between \of{f} and \of{fold}
results and, accordingly, updates the table and the accumulator. For a
\of{f} result, either we combine it with the accumulator in a new
...
combine it with a pending \of{f} result from the table.
...
forms.
\begin{ocaml}
  Worker.map_fold_ac : f:('a -> 'b) -> fold:('b -> 'b -> 'b) -> unit -> unit
